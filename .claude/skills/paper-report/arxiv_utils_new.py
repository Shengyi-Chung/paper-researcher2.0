...
(if YY >= 23), scan **Introduction**, **Methods / Methodology** (best effort), **Conclusion**
(and related headings), and the **References** bibliography for arXiv-style ids,
and build a directed citation graph:
...
For report skills: use ``fetch_paper_export_bundle`` to obtain introduction,
methodology, conclusion plain text, and the filtered related-id list from a single
HTML fetch.
"""
...
def extract_methodology_html(page: str) -> str:
    """First section whose heading matches common methods/methodology titles."""
...
def fetch_paper_export_bundle(
    ...
    *,
    ...
    methodology_max_chars: int = 12000,
    conclusion_max_chars: int = 8000,
) -> dict:
    """
    One HTTP fetch: introduction + methodology + conclusion HTML (best effort),
    references, plain-text excerpts (truncated), and related arXiv ids from all
    of the above (including methodology body text for citation discovery).

    Keys: canonical_id, base_id, introduction_text, methodology_text,
    conclusion_text, related_arxiv_ids_recent, has_references_section.
    """
    url = EXPORT_HTML_BASE + arxiv_export_id
    page = fetch_html(url)
    intro_h = extract_introduction_html(page)
    meth_h = extract_methodology_html(page)
    ...
    meth_t = html_fragment_to_text(meth_h) if meth_h else ""
    ...
    if methodology_max_chars > 0 and len(meth_t) > methodology_max_chars:
        meth_t = meth_t[:methodology_max_chars].rsplit(" ", 1)[0] + " …"
    if conclusion_max_chars > 0 and len(concl_t) > conclusion_max_chars:
        concl_t = concl_t[:conclusion_max_chars].rsplit(" ", 1)[0] + " …"
    blob = "\n".join(x for x in (intro_h, meth_h, concl_h, bib_h) if x)
...
def build_citation_graph(raw_ids: list[str]) -> nx.DiGraph:
    ...
    that paper's export HTML (introduction, methodology, conclusion/discussion,
    and references),
